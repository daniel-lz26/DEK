...
import { 
  ...
  ForwardIcon, 
  BackwardIcon, 
  SpeakerWaveIcon, // 1. Import volume icon
  SpeakerXMarkIcon // 2. Import mute icon
} from '@heroicons/react/24/solid';
import albumArt from '../images/kendrick-gnx.jpg';

export const MusicPlayer = () => {
  const [isPlaying, setIsPlaying] = React.useState(false);
  const [volume, setVolume] = React.useState(75); // 3. Add state for volume (0-100)
    ...
    <div className="fixed bottom-0 left-0 w-full bg-neutral-800 text-white px-2 py-2 shadow-lg z-50">
      <div className="max-w-7xl mx-auto flex items-center justify-between">
        ...
        <div className="w-1/4 flex items-center justify-end gap-2">
          <button onClick={() => setVolume(volume > 0 ? 0 : 75)} className="text-neutral-400 hover:text-white transition-colors">
            {volume > 0 ? <SpeakerWaveIcon className="h-5 w-5"/> : <SpeakerXMarkIcon className="h-5 w-5"/>}
          </button>
          <input 
            type="range" 
            min="0" 
